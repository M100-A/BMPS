...
#endif
{
    ...
    // I wanted to perform a sanity check on the schedule data early.
    // If there is a mistake with the schedule data, the program will not
...
#ifdef __PDCURSES__
    PDC_set_title( "Bunny, Mayoress, Prostitute & Succubus v0.82");
#endif
    ...
    while (1)
    {
        ...
        //
        if (cLogEnable != 0)
            futils_writeFileHeaderAndData ("BMPS-Log.txt", "#BMPS v0.82 log output\n", gpszDialogBuffer);
...
//                if (iKey == 19)
            ...
            else if (cKeyMode == 10)
            {
                ...
                if (((iKey >= 'A') && (iKey <= 'Z')) || ((iKey >= 'a') && (iKey <= 'z')) || ((iKey >= '0') && (iKey <= '9')) || (iKey == '-') || (iKey == ',') || (iKey == '&') || (iKey == ' ') || (iKey == '(') || (iKey == ')') || (iKey == '\''))
                {
