...
//
// sub-function used by httppost_urlConvertList ().
//
char httppost_fromHex (char ch)
{
...
void httppost_urlConvertList (char *pPostData, char *pOutput, int iLen)
{
    ...
    while (pTemp[0] != 0)
    {
        ...
        if (pTemp[0] == '%')
        {
            if (pTemp[1] && pTemp[2])
            {
                pOutput[i] = httppost_fromHex(pTemp[1]) << 4 | httppost_fromHex(pTemp[2]);
                pTemp += 2;
            }
        }
        else if (pTemp[0] == '+')
...
//
// To work with the converted name-value data (pre-formatted by httppost_urlConvertList),
